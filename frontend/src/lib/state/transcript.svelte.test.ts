...
describe('TranscriptState', () => {
	...
	describe('updateOrAddPartial', () => {
		it('adds a new partial entry on first call', () => {
			transcriptState.updateOrAddPartial('Hello', 'cs');

			expect(transcriptState.entries).toHaveLength(1);
			expect(transcriptState.entries[0].text).toBe('Hello');
			expect(transcriptState.entries[0].isFinal).toBe(false);
		});

		it('updates existing partial in-place', () => {
			transcriptState.updateOrAddPartial('Hello', 'cs');
			transcriptState.updateOrAddPartial('Hello world', 'cs');
	...
	describe('finalizePartial', () => {
		it('marks current partial as final', () => {
			transcriptState.updateOrAddPartial('Hello', 'cs');
			transcriptState.finalizePartial('Hello world');
		...
		it('adds new entry if no partial exists', () => {
			transcriptState.finalizePartial('Hello world');
		...
		it('allows new partial after finalization', () => {
			transcriptState.updateOrAddPartial('First', 'cs');
			transcriptState.finalizePartial('First sentence');
			transcriptState.updateOrAddPartial('Second', 'cs');

			expect(transcriptState.entries).toHaveLength(2);
			expect(transcriptState.entries[0].isFinal).toBe(true);
			expect(transcriptState.entries[1].isFinal).toBe(false);
		});
	});
});
