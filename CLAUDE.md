...
- **WebSocket protocol** (routes/websocket.py): `connected` → `configure` → `ready` → binary audio / `partial` results → `stop` → `final` results → `done`. Buffers audio and transcribes every 2s of new data. Force-finalizes at 5s.
